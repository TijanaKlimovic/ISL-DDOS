...
func ParseUDPAddr(s string) (*UDPAddr, error) {
	rawIA, rawHost, err := parseAddr(s)	//strings just separated rawIA and rawHost from s
	if err != nil {
		return nil, err
	}
	ia, err := addr.IAFromString(rawIA)	//encode in number to place in the addr.IA structure
	if err != nil {
	...
	rawIP, rawPort, err := net.SplitHostPort(rawHost)	//rawIP and port are again just split strings from rawHost
	if err != nil {
		return nil, err
	}
	ip := net.ParseIP(rawIP)	//IP 
	if ip == nil {
